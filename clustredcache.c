...
*/
		...
		case TYPE_PINGREQ:
			...
			if (peer) {
				...
				// Set Default Reply
buf[0] = TYPE_PINGRPL;
int hello_send_len = 9; // default: PINGRPL is always 9 bytes
...
buf[14] = cache->port;
fase( buf+11, buf+6);
/* Append TLV fields so oscam peers can identify us.
   Format: tag(1) + len(1) + value(len)
   0x01 = program name, 0x02 = version string          */
static const char csp_prog[] = "multics";
static const char csp_ver[]  = REVISION_STR;
int tl = 9; // TLV write position
buf[tl]   = 0x01;
buf[tl+1] = (uint8_t)(sizeof(csp_prog)-1);
memcpy(buf+tl+2, csp_prog, sizeof(csp_prog)-1);
tl += 2 + (int)(sizeof(csp_prog)-1);
buf[tl]   = 0x02;
buf[tl+1] = (uint8_t)(sizeof(csp_ver)-1);
memcpy(buf+tl+2, csp_ver, sizeof(csp_ver)-1);
tl += 2 + (int)(sizeof(csp_ver)-1);
hello_send_len = tl;
}
} else if ( !peer->csp || (received>13) ) { peer->flags |= FLAG_DISABLE; break; }
sendtopeer( peer, buf, hello_send_len);
				// Check for activity
