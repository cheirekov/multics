...
#ifdef CHECK_NEXTDCW
static inline int cwcycle_action_drop( struct cardserver_data *cs, ECM_DATA *ecm )
{
	if (!cs->option.dcw.cwcycle) return 1;

	if (cs->option.dcw.cwcycle_action!=1) {
		const char *action = (cs->option.dcw.cwcycle_action==2) ? "log" : "drop";
		mlogf(LOGWARNING,0," [CWCYCLE] %s repeated/invalid CW %04x:%06x:%04x/%02x:%08x\n",
			action, ecm->caid, ecm->provid, ecm->sid, ecm->ecm[0], ecm->hash );
	}

	return (cs->option.dcw.cwcycle_action==0);
}
#endif
...
#ifdef CHECK_NEXTDCW
	if ((cs->option.dcw.check || cs->option.dcw.cwcycle) && !cs->option.dcw.halfnulled) {
		int check = checkfreeze_setdcw(ecm,dcw);
		if (check==0) {
			ecm->lastdecode.error++;
			if (cwcycle_action_drop(cs, ecm)) return;
		}
		else if (check==1) {
