...
#ifdef CCCAM
//2.3.0 build 3367 (latest supported — oscam-compatible)
strcpy(cfg->cccam.version, cc_version[4]);
sprintf(cfg->cccam.build, "%d", cc_build[4]);
...
#ifdef FREECCCAM_SRV
//2.3.0 build 3367 (latest supported — oscam-compatible)
strcpy(cfg->freecccam.version, cc_version[4]);
sprintf(cfg->freecccam.build, "%d", cc_build[4]);
...
cs->option.server.lb_blocktime = 0; /* Phase 3: LB block disabled by default */
cs->option.server.lb_mincount  = 3; /* Phase 3: block after 3 consecutive timeouts */
...
///////////////////////////////////////////////////////////////////////////////
			...
			if (!strcmp(str,"KEY")) {
				...
				if (!strcmp(str,"TIMEOUT")) {
				...
				else if (!strcmp(str,"CWCYCLE")) {
					parse_spaces();
					if ((*iparser!=':')&&(*iparser!='=')) {
						mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%d,%d): ':' expected\n",file->nbline,iparser-currentline);
						continue;
					} else iparser++;
					defaultcs.option.dcw.cwcycle = parse_boolean();
				}
				else if (!strcmp(str,"CWCYCLE_ACTION")) {
					parse_spaces();
					if ((*iparser!=':')&&(*iparser!='=')) {
						mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%d,%d): ':' expected\n",file->nbline,iparser-currentline);
						continue;
					...
					parse_name(str);
					uppercase(str);
					if (!strcmp(str,"ACCEPT")) defaultcs.option.dcw.cwcycle_action = 1;
					else if (!strcmp(str,"LOG")) defaultcs.option.dcw.cwcycle_action = 2;
					else defaultcs.option.dcw.cwcycle_action = 0;
				}
#endif
			}
			else if ( !strcmp(str,"SERVER") ) {
...
else if (!strcmp(str,"LB_BLOCKTIME")) {
parse_spaces();
if ((*iparser!=':')&&(*iparser!='=')) {
mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%d,%d): ':' expected\n",file->nbline,iparser-currentline);
continue;
...
parse_int(str);
defaultcs.option.server.lb_blocktime = (uint32_t)atoi(str);
}
else if (!strcmp(str,"LB_MINCOUNT")) {
parse_spaces();
if ((*iparser!=':')&&(*iparser!='=')) {
mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%d,%d): ':' expected\n",file->nbline,iparser-currentline);
continue;
...
parse_int(str);
defaultcs.option.server.lb_mincount = (uint32_t)atoi(str);
if (!defaultcs.option.server.lb_mincount) defaultcs.option.server.lb_mincount = 1;
}
...
///////////////////////////////////////////////////////////////////////////////
			...
			if (!strcmp(str,"TIMEOUT")) {
				...
				cardserver->option.dcw.cwcycle = parse_boolean();
			}
			else if (!strcmp(str,"CWCYCLE_ACTION")) {
				parse_spaces();
				if ((*iparser!=':')&&(*iparser!='=')) {
				...
				parse_name(str);
				uppercase(str);
				if (!strcmp(str,"ACCEPT")) cardserver->option.dcw.cwcycle_action = 1;
				else if (!strcmp(str,"LOG")) cardserver->option.dcw.cwcycle_action = 2;
				else cardserver->option.dcw.cwcycle_action = 0;
			}
...
parse_int(str);
cardserver->option.server.lb_blocktime = (uint32_t)atoi(str);
}
else if (!strcmp(str,"LB_MINCOUNT")) {
parse_spaces();
if ((*iparser!=':')&&(*iparser!='=')) {
...
parse_int(str);
cardserver->option.server.lb_mincount = (uint32_t)atoi(str);
if (!cardserver->option.server.lb_mincount) cardserver->option.server.lb_mincount = 1;
}
else if (!strcmp(str,"FIRST")) {
parse_spaces();
				if ((*iparser!=':')&&(*iparser!='=')) {
