...
int cs_msg_peek(int handle,struct cs_custom_data *cd, unsigned char *buffer, unsigned char *deskey)
{
	int len;
	unsigned char netbuf[CWS_NETMSGSIZE];
	int returnLen;
	...
	//
	len = recv(handle, netbuf, 2, MSG_NOSIGNAL|MSG_DONTWAIT);
	if ( (len<0) && ( (errno==EINTR)||(errno==EWOULDBLOCK)||(errno==EAGAIN) ) )	len = recv(handle, netbuf, 2, MSG_NOSIGNAL|MSG_DONTWAIT);
	...
	if (netlen>(CWS_NETMSGSIZE-2)) return -1;
	//
	len = recv(handle, netbuf+2, netlen, MSG_NOSIGNAL|MSG_DONTWAIT);
	if ( (len<0) && ( (errno==EINTR)||(errno==EWOULDBLOCK)||(errno==EAGAIN) ) )	len = recv(handle, netbuf+2, netlen, MSG_NOSIGNAL|MSG_DONTWAIT);
	if (len<=0) return len; // disconnected
	if (len!=netlen) return -2;
	//
	len += 2;
	len= des_decrypt(netbuf, len, deskey);
	if (len < 15) {
