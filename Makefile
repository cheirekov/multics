...
# Docker targets
DOCKER_IMAGE ?= multics
DOCKER_TARGET ?= x64
DOCKER_EXTRA_OPTS ?=

docker-build:
	docker build --target builder \
	  --build-arg TARGET=$(DOCKER_TARGET) \
	  --build-arg EXTRA_OPTS="$(DOCKER_EXTRA_OPTS)" \
...
docker-test:
	docker build --target tester \
	  --build-arg TARGET=$(DOCKER_TARGET) \
	  --build-arg EXTRA_OPTS="$(DOCKER_EXTRA_OPTS)" \
	  -t $(DOCKER_IMAGE):tester .
	docker run --rm $(DOCKER_IMAGE):tester
...
docker-all: docker-build docker-test docker-runtime
