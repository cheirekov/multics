...
struct cardserver_data
{
	...
	struct {
		...
		struct {
			...
			/* Phase 4: CW cycle check */
			uint8_t cwcycle;        /* 1: enable CW cycle/freeze detection (default: off) */
			uint8_t cwcycle_action; /* 0=drop, 1=accept, 2=log repeated/invalid CW */
		} dcw;
...
/* Phase 3: LB blocking */
uint32_t lb_blocktime;  /* ms to block server after lb_mincount consecutive timeouts (0=disabled) */
uint32_t lb_mincount;   /* consecutive ECM timeouts before block is applied (0=use default 3) */
#ifndef PUBLIC
...
/* Phase 3: LB blocking — per-server runtime state */
uint32_t lb_blocked_until; /* GetTickCount() expiry timestamp; 0=not blocked */
int lb_fails;              /* consecutive ECM timeout count since last success */
