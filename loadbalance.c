...
*/
#ifdef CAID_BULCRYPT_PROV
if ( ((card->caid & 0xff00)!=0x0100) && ((card->caid & 0xff00)!=0x0500)
    && card->caid!=CAID_BULCRYPT1 && card->caid!=CAID_BULCRYPT2 ) return 1;
#else
...
// Provider-aware matching: Seca, Viaccess, and (when enabled) Bulcrypt
#ifdef CAID_BULCRYPT_PROV
if ( ((card->caid & 0xff00)!=0x0100) && ((card->caid & 0xff00)!=0x0500)
    && card->caid!=CAID_BULCRYPT1 && card->caid!=CAID_BULCRYPT2 ) return 1;
#else
...
srv->busy = 0;
disconnect_srv(srv);
/* Phase 3: LB blocking — count consecutive timeouts */
if (cs->option.server.lb_blocktime > 0) {
uint32_t mincount = cs->option.server.lb_mincount ? cs->option.server.lb_mincount : 3;
srv->lb_fails++;
if ((uint32_t)srv->lb_fails >= mincount) {
srv->lb_blocked_until = ticks + cs->option.server.lb_blocktime;
srv->lb_fails = 0;
mlogf(LOGWARNING,getdbgflag(DBG_SERVER,0,srv->id)," LB: server (%s:%d) blocked for %ums after %u consecutive timeouts\n", srv->host->name, srv->port, cs->option.server.lb_blocktime, mincount);
}
}
}
else {
						...
						if (i==0) {
							...
							/* Phase 3: skip LB-blocked servers; auto-unblock on expiry */
if (srv->lb_blocked_until) {
if (ticks < srv->lb_blocked_until) {
pthread_mutex_unlock(&srv->lock);
srv = srv->next; continue;
}
srv->lb_blocked_until = 0; srv->lb_fails = 0; /* block expired */
}
